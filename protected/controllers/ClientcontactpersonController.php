...
class ClientcontactpersonController extends RController
{
	...
	public function actionView($id)
	{
		...
		$getClientId = Clientcontactperson::model()->findByPk($id);
		$empId = Client::model()->findByPk($getClientId->clientId);
		
		$params['employeeId'] = $empId->employeeId;
		if (Yii::app()->user->checkAccess('Client Contact Own', $params) || Yii::app()->user->checkAccess('Admin'))
		{
		...
		if(isset($_POST['Clientcontactperson']))
		{
			...
			if($model->save()){
				if(isset($_GET['clientId']))
					$this->redirect(array('clientintake/create','clientId'=>$model->clientId, 'success'=>'clientContact'));
				else
					$this->redirect('../client/'.$model->clientId);
			}
		}
